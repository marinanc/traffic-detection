...
substraction = cv2.createBackgroundSubtractorKNN() #search history y varTreshold
...
while True:
    ...
    if cv2.countNonZero(mask) > 0:
	    for(i,c) in enumerate(contour):
	        ...
	        for(x,y) in detec:
	        	if (y<(pos_line1+offset) and y>(pos_line1-offset) and x<1100 and x>700) or (y<(pos_line2+offset) and y>(pos_line2-offset) and x<900 and x>0) or (y<(pos_line3+offset) and y>(pos_line3-offset) and x<1850 and x>1100):
	        		vehicle+=1
	        		...
	        		if y<(pos_line2+offset) and y>(pos_line2-offset) and x<900 and x>0:
	        			cv2.line(frame, (0, pos_line2-30), (900, pos_line2), (0,0,255), 12)
	        		if y<(pos_line3+offset) and y>(pos_line3-offset) and x<1850 and x>1100:
